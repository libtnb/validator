...
func TestPointerScalarFields(t *testing.T) {
	...
	age, name := 20, "alice"
	ok := MustStruct(T{Age: &age, Name: &name})
	...
	bad := 10
	low := MustStruct(T{Age: &bad, Name: &name})
...
func TestDerefTypeRecursivePointerNoHang(t *testing.T) {
	completesWithin(t, "derefType recursive pointer", func() {
		...
		MustStruct(S{}).Validate(context.Background())
	})
}
...
func TestSourceOfRecursivePointerNoHang(t *testing.T) {
	completesWithin(t, "MustValue(recursive ptr)", func() {
		type P *P
		var p P
		p = &p
		MustValue(p, "").Validate(context.Background())
	})
	completesWithin(t, "MustStruct(recursive ptr)", func() {
		type P *P
		var p P
		p = &p
		MustStruct(p).Validate(context.Background())
	})
}
...
func TestLegalPointerChainsStillWork(t *testing.T) {
	...
	var out S
	if err := MustMap(map[string]any{"N": 5}, nil).Bind(&out); err != nil {
...
func TestCyclicValueValidationNoCrash(t *testing.T) {
	m := map[string]any{}
	m["self"] = m
	vd := MustMap(map[string]any{"x": m}, map[string]string{"x": "notblank"})
	vd.Validate(context.Background())
	_ = vd.Fails()

	s := make([]any, 1)
	s[0] = s
	dv := MustMap(map[string]any{"list": []any{s}}, map[string]string{"list": "dive && notblank"})
	dv.Validate(context.Background())
	_ = dv.Fails()

	dm := MustMap(map[string]any{"m": map[string]any{"k": s}}, map[string]string{"m": "dive && notblank"})
	dm.Validate(context.Background())
	_ = dm.Fails()
}

// Flat sources (Map/Value) dereference pointer values like the struct path.
func TestFlatSourcePointerDeref(t *testing.T) {
	n := 42
	vd := MustMap(map[string]any{"age": &n}, map[string]string{"age": "numeric && gte:18"})
	vd.Validate(context.Background())
	...
	var np *int
	vd2 := MustMap(map[string]any{"age": np}, map[string]string{"age": "required"})
	...
	vv := MustValue(&n, "numeric && gte:18")
	vv.Validate(context.Background())
	if vv.Errors().Has("value") {
		t.Errorf("MustValue(*int) should deref, got %v", vv.Errors().All())
	}
}
...
func TestNilSafety(t *testing.T) {
	vd := MustMap(map[string]any{"Addr": nil}, map[string]string{"Addr.City": "required"})
	...
	cf := MustMap(map[string]any{"A": 5, "B": nil}, map[string]string{"A": "gtfield:B"})
...
func TestDerefConsistencyAcrossEntryPoints(t *testing.T) {
	...
	cases := map[string]*Validation{
		"struct *int":      MustStruct(SP{Age: &n}),
		"struct any->*int": MustStruct(SA{Age: &n}),
		"map *int":         MustMap(map[string]any{"Age": &n}, map[string]string{"Age": "numeric && gte:18"}),
		"var *int":         MustValue(&n, "numeric && gte:18"),
		"dive []*int":      MustMap(map[string]any{"xs": []*int{&n}}, map[string]string{"xs": "dive && numeric && gte:18"}),
	}
...
func TestSelfRefAnyFieldNoHang(t *testing.T) {
	...
	go func() {
		MustStruct(S{V: v}).Validate(context.Background())
		done <- true
	}()
	select {
	case <-done:
...
func TestNilableRequiredConsistency(t *testing.T) {
	...
	ok := MustStruct(T{S: []string{}, M: map[string]int{}})
	ok.Validate(context.Background())
	if ok.Fails() {
	...
	zok := MustStruct(Z{}) // zero string is non-nil-able -> present
...
func TestNilUnsafePointerRequired(t *testing.T) {
	...
	x := 1
	ok := MustStruct(T{P: unsafe.Pointer(&x)})
	ok.Validate(context.Background())
	if ok.Fails() {
